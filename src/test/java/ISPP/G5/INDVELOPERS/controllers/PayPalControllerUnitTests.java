...
import ISPP.G5.INDVELOPERS.paypal.OrderService;
...
class PayPalControllerUnitTests {
	...
	void testPayment() throws Exception {
		String bodyContent = objectToJsonStringContent(order);
		Payment payment = new Payment();
		payment.setLinks(new ArrayList<Links>());
		when(service.createPayment(any(Double.class), any(String.class), any(String.class), any(String.class),
				any(String.class), any(String.class), any(String.class), any(String.class))).thenReturn(payment);

		mvc.perform(post("/payments/pay").contentType(MediaType.APPLICATION_JSON).content(bodyContent))
				.andExpect(status().isOk());

	}

	@Test
	@DisplayName("Success pay Test")
	@WithMockUser(value = "spring")
	void testSuccessPay() throws Exception {
		Payment payment = new Payment();
		payment.setState("approved");
		when(service.executePayment(any(String.class), any(String.class))).thenReturn(payment);
		when(developerService.findByUsername(any(String.class))).thenReturn(developer);
		when(ownedGameService.buyGameByDeveloperAndGameId(any(Developer.class), any(String.class))).thenReturn("OK");

		mvc.perform(get("/payments/success?paymentId=algo&PayerID=otro&gameId=mismo")).andExpect(status().isOk());
	...
	void testBuy() throws Exception {
		...
		payment.setState("approved");
		payment.setLinks(new ArrayList<Links>());
		when(service.createPaymentToUs(any(Double.class), any(String.class), any(String.class), any(String.class),
				any(String.class), any(String.class), any(String.class))).thenReturn(payment);
		when(orderService.save(any(Order.class))).thenReturn(order);
		when(developerService.findCurrentDeveloper()).thenReturn(developer);

		mvc.perform(post("/payments/subscription")).andExpect(status().isOk());

	}

	@Test
	@DisplayName("Subscription Success Test")
	@WithMockUser(value = "spring")
	void testSubscriptionSuccess() throws Exception {
		Payment payment = new Payment();
		payment.setState("approved");
		payment.setLinks(new ArrayList<Links>());
		when(service.executePayment(any(String.class), any(String.class))).thenReturn(payment);
		when(developerSubService.buySubscription(any(Developer.class))).thenReturn("Ok");
		when(developerService.findCurrentDeveloper()).thenReturn(developer);

		mvc.perform(get("/payments/subscriptionSuccess?paymentId=algo&PayerID=algo")).andExpect(status().isCreated());
